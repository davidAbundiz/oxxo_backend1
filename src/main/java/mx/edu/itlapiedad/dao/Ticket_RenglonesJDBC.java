...
public class Ticket_RenglonesJDBC implements Ticket_RenglonesDAO{
	...
	public void insertar(Ticket_Renglones ticket_renglones) {
		...
		sql = "INSERT INTO ticket_renglones (TICKET_id, PRODUCTO_id, cantidad, precio, importe) VALUES (?, ?, ?, ?, ?)";
		oxxo_db.update(sql, ticket_renglones.getTICKET_id(), ticket_renglones.getPRODUCTO_id(), ticket_renglones.getCantidad(), ticket_renglones.getPrecio(), ticket_renglones.getImporte());
	}
	@Override
	...
	public void actualizar(Ticket_Renglones ticket_renglones) {
		...
		String sql = "UPDATE ticket_renglones SET TICKET_id = ?,"
				+" PRODUCTO_id = ?, cantida = ?, precio = ?, importe = ?"
				+" WHERE id=?";
		oxxo_db.update(sql, ticket_renglones.getId(), ticket_renglones.getTICKET_id(), 
